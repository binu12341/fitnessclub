...
from fitnessproj.settings import EMAIL_HOST_USER
...
def userpaymentpage(request):
	if 'Tne_id' in request.session:
		...
		if request.method=='POST':
			...
			n=request.POST.get('bankname')
			an=request.POST.get('accnumber')
			ifsc=request.POST.get('ifsccode')
			am=request.POST.get('payment')
			date = datetime.now()
...
def offlineedit(request,offd_id):
	if 'Tnr_id' in request.session:
		...
		if request.method=='POST':
			...
			offds.save()
			subject = 'FitnessClub Joining Details'
			message = 'dear Friend,\nWe are pleased to inform that your request to join our online class is approved'
...
def admregedit(request,i_id):
	if 'SAdm_id' in request.session:
		...
		return render(request, 'adm_regedit.html',{'users':users,'reg':reg})
	else:
		return redirect('/')		
...
def admin_pay_page(request):
	if 'SAdm_id' in request.session:
		...
		if request.method=='POST':
			...
			am=request.POST['payment']
			date = datetime.now()
			paymenttrainee.objects.create(name=n,sname=sn,accountnumber=an,ifsc=ifsc,payment=am,date=date)
			msg_success = "Payment successfully Completed"
